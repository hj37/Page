...
public class BoardDAO {
	...
	//매개변수로 전달받은 section과 pageNum값을 이용해 SQL문으로 글조회 메소드
	public List	selectAllArticles(Map pagingMap) {
		...
		try{
		   ...
		   while(rs.next()){
			...
			System.out.println("selectAllArticles메소드 내부에서 예외발생:" + e);
		}		
		return articlesList; //BoardService로 리턴
	}//메소드 끝
	...
	public int selectToArticles() {
		...
		try {
			...
			System.out.println("selectToArticles메소드에서 예외발생 :" + e);
		}
		return 0; //BoardService로 리턴
	}
