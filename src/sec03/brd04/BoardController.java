...
/*
		    ...
		    사장 (BoardController) -> 부장(BoardService) -> 사원(BoardDAO)
	View
고객(웹브라우저)<-	검색한 하나의 글정보	<-검색한 하나의 글정보	 <-검색한 하나의 글정보
*/


//  /viewArticle.do로 요청하여 글번호를 받아옵니다.
//  그리고 그번호에 해당하는 글정보를 검색해서 가져와 request객체메모리에 article속성이름으로 바인딩한후
//  viewArticle.jsp로 전송합니다.
//@WebServlet("/board/*")
...
//업로드한 파일명을 제외한~ 입력한 글제목,입력한 글내용을 key/value 형태의 값들로 저장되어 있는 HashMap을 리턴 받는다.
		...
		}else if(action.equals("/viewArticle.do")) {//글을 수정하기전에 하나의 글정보를 검색하여
													//글 상세화면에 출력 해줘~ 라는 요청주소를 받았을떄
				
			//1.서블릿은 클라이언트가 요청한 값(글번호)을 얻어야 함
			String articleNO = request.getParameter("articleNO");
			
			//2.요청한 글번호에 대한 글정보를 조회 하기 위해 
			//BoardService객체의 viewArticle(int articleNO)메소드 호출시...
			//요청한 글번호를 전달 하여 글조회 명령을 하게 됨
			//그리고 글을 조회한후 하나의 글정보를 반환받습니다.
			articleVO = boardService.viewArticle(Integer.parseInt(articleNO));
			
			//3.View페이지로 응답결과물(조회한 하나의 글정보)를 전달하기위해
			//request임시 메모리 공간에 저장
			request.setAttribute("article", articleVO);
			
			//4.View페이지(조회한 글 하나의 정보를 출력할 페이지)의 경로주소를 저장
